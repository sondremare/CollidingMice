...
//! [22] //! [23]
        ...
        private double grade(double x, double leftEndpoint, double rightEndpoint) {
            if (x <= leftEndpoint) {
                return 0;
            } else if (x >= rightEndpoint) {
                return 1;
            } else {
                double slopeWidth = rightEndpoint - leftEndpoint;
                return (x / slopeWidth) - (leftEndpoint / slopeWidth);
            }
        }

        private double reverseGrade(double x, double leftEndpoint, double rightEndpoint) {
            if (x <= leftEndpoint) {
                return 1;
            } else if (x >= rightEndpoint) {
                return 0;
                ...
                double slopeWidth = rightEndpoint - leftEndpoint;
                return (-x / slopeWidth) + (rightEndpoint / slopeWidth);
            }
        }

        private double triangle(double x, double leftEndpoint, double centerPoint, double rightEndpoint) {
            if (x == centerPoint) {
                return 1;
            }
            else if (x <= leftEndpoint || x >= rightEndpoint) {
                return 0;
            } else if (leftEndpoint < x && x < centerPoint) {
                double upwardSlopeWidth = centerPoint - leftEndpoint;
                return (x / upwardSlopeWidth) - (leftEndpoint / upwardSlopeWidth);
            } else {
                double downWardSlopeWidth = rightEndpoint - centerPoint;
                return (-x / downWardSlopeWidth) + (rightEndpoint / downWardSlopeWidth);
            }
        }
//! [23]
    }
